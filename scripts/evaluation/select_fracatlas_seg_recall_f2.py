...
RUN_DIR = ROOT / "runs" / "segment" / "fracatlas_yolo11s_seg_recall_v2"
...
SCREENING_CSV = RUN_DIR / "f2_checkpoint_screening.csv"
OUTPUT_CSV = RUN_DIR / "f2_checkpoint_selection.csv"
OUTPUT_JSON = RUN_DIR / "f2_checkpoint_selection.json"
...
def validate_checkpoint(weight_path, device, batch, run_name):
    ...
    result = model.val(
        ...
        iou=0.70,
        plots=False,
        project=str(RUN_DIR / "f2_validation_runs"),
        name=run_name,
        exist_ok=True,
        verbose=False,
    )
...
def main():
    ...
    candidates.extend(
        (
            f"recall_v2_epoch_{index + 1:02d}",
            index + 1,
            weight_path,
        )
